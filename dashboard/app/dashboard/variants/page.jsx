...
export default function VariantsPage() {
  ...
  useEffect(() => { if (selected) fetchVariants(selected); else setVariants([]); }, [selected]);
    ...
    try {
      if (editing) {
        ...
        await fetch(`${API}/api/admin/variants`, {
          ...
          body: JSON.stringify({ product_id: parseInt(selected), ...form, price: parseInt(form.price) }),
        });
      }
  ...
  async function handleDelete(id) {
    if (!confirm('Hapus varian ini beserta semua stoknya?')) return;
    await fetch(`${API}/api/admin/variants/${id}`, { method: 'DELETE', headers: authHeaders() });
    fetchVariants(selected);
  }

  async function handleToggle(v) {
    await fetch(`${API}/api/admin/variants/${v.id}`, {
      method: 'PUT', headers: authHeaders(),
  ...
  function startEdit(v) {
    setEditing(v.id);
    setForm({ name: v.name, description: v.description || '', price: String(v.price), terms: v.terms || '' });
  ...
  return (
    <div>
      ...
      <p className="text-gray-500 text-sm mb-6">Kelola varian untuk setiap produk (contoh: 1 Bulan, 1 Tahun).</p>
      ...
      <div className="bg-white rounded-2xl shadow p-4 md:p-6 mb-4">
        ...
        <select
          ...
          value={selected}
          onChange={e => { setSelected(e.target.value); setEditing(null); setForm({ name: '', description: '', price: '', terms: '' }); }}
        >
          ...
          <div className="bg-white rounded-2xl shadow p-4 md:p-6 mb-4">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <input
                ...
                placeholder="Nama varian (e.g. 1 Bulan, 1 Tahun - 1 Akun)"
                value={form.name}
                onChange={e => setForm({ ...form, name: e.target.value })}
              />
              <input
                className="border rounded-lg px-3 py-2.5 text-sm"
                placeholder="Harga (IDR e.g. 15000)"
                type="number"
                value={form.price}
                onChange={e => setForm({ ...form, price: e.target.value })}
              />
              <textarea
                className="border rounded-lg px-3 py-2.5 text-sm sm:col-span-2"
                rows={2}
              ...
              <div className="sm:col-span-2">
                <label className="block text-xs font-medium text-gray-500 mb-1">
                  📋 S&K Varian <span className="text-gray-400">(opsional — override S&K produk)</span>
                </label>
                <textarea
                  className="border rounded-lg px-3 py-2.5 text-sm w-full"
                  rows={3}
                  placeholder="Kosongkan untuk pakai S&K produk"
                  value={form.terms}
              ...
              <tbody className="divide-y divide-gray-100">
                {variants.map(v => (
                  <tr key={v.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-500">{v.sold_count || 0}</td>
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${v.terms ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-400'}`}>
                        {v.terms ? 'Custom' : 'Default'}
                      </span>
                    </td>
                ...
                {!variants.length && (
                  <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-400">Belum ada varian.</td></tr>
                )}
              </tbody>
            </table>
          </div>
              ...
              <div key={v.id} className="bg-white rounded-2xl shadow p-4">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div>
                    ...
                    <p className="text-blue-600 font-medium text-sm">Rp {Number(v.price).toLocaleString('id-ID')}</p>
                  </div>
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${v.is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                    {v.is_active ? 'Aktif' : 'Nonaktif'}
                  </span>
                ...
                <div className="flex gap-4 text-sm text-gray-500 mb-3">
                  <span>✅ {v.stock_count || 0} stok</span>
                  <span>📦 {v.sold_count || 0} terjual</span>
                </div>
